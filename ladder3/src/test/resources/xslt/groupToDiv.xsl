...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:p">
        <xsl:variable name="beforeWithPattern">
            <xsl:choose>
                <xsl:when test="preceding::tei:p[matches(string(.), '^(AUS|ita)_(\d+_)?(\d+).*$')]">
                    <xsl:text>yes</xsl:text>
                </xsl:when>
                <xsl:otherwise>no</xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="nextWithPattern" select="following::tei:p[matches(string(.), '^(AUS|ita)_(\d+_)?(\d+).*$')][1]"/>
        <xsl:variable name="parent" select="parent::tei:*[1]"/>
        <xsl:choose>
            <xsl:when test="matches(string(.), '^(AUS|ita)_(\d+_)?(\d+).*$')">
                <div>
    ...
    <xsl:template name="copyBetween">
        ...
        <xsl:variable name="myEnd">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:copy-of select="$parent/tei:p[position()=last()]/generate-id()"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
